...
export const getReview: (query: ReviewQuery) => Promise<AxiosResponse<Review, any>> = (query: ReviewQuery) => {
  return axios.get<Review>(
    `http://localhost:8080/reviewData?reviewerId=${query.reviewerId}&projectId=${query.projectId}&dataId=${query.dataNumber}`,
  );
};

export const getReviewForm: (reviewerId: number, projectId: number) => Promise<AxiosResponse<FormComponent, any>> = (
  reviewerId: number,
  projectId: number,
) => {
